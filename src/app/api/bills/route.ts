...
// GET bills - with optional categories mode
export async function GET(request: NextRequest) {
  try {
    ...
    if (searchParams.get('mode') === 'categories') {
      const categories = await db.billCategory.findMany({
        ...
        include: {
          ...
          _count: { select: { bills: { where: { isPublished: true } } } },
        },
        where: { parentId: null },
      });
    ...
    const [bills, total, categories] = await Promise.all([
      db.bill.findMany({
        where,
        include: {
          category: {
